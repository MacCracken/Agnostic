...
class HistoryManager:
            ...
            for key in session_keys:
                ...
                if session_data:
                    try:
                        ...
                        # Calculate duration
                        if completed_at:
                            duration = int((completed_at - created_at).total_seconds() / 60)
                        else:
                            duration = int((datetime.now() - created_at).total_seconds() / 60)
                        ...
                        sessions.append(SessionSummary(
                            ...
                            scenarios_completed=metrics.get("scenarios_completed", 0),
    ...
    async def get_session_details(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Cache the result
            self.redis_client.setex(cache_key, self.cache_timeout, json.dumps(session_data, default=str))
    ...
    async def compare_sessions(self, session1_id: str, session2_id: str) -> Optional[SessionComparison]:
        ...
        try:
            ...
            comparison_metrics = {
                "overall_scores": {
                    ...
                    "difference": (metrics2.get("overall_score", 0) - metrics1.get("overall_score", 0))
                },
                "test_coverage": {
                    "session1": metrics1.get("test_coverage"),
                    "session2": metrics2.get("test_coverage"),
                    "difference": (metrics2.get("test_coverage", 0) - metrics1.get("test_coverage", 0))
                },
                "duration": {
                    "session1": metrics1.get("duration_minutes"),
                    "session2": metrics2.get("duration_minutes"),
                    "difference": (metrics2.get("duration_minutes", 0) - metrics1.get("duration_minutes", 0))
                },
                "agent_count": {
                    "session1": metrics1.get("total_agents"),
                    "session2": metrics2.get("total_agents"),
                    "difference": (metrics2.get("total_agents", 0) - metrics1.get("total_agents", 0))
                }
            }
            ...
            if abs(comparison_metrics["overall_scores"]["difference"] or 0) > 5:
                differences.append({
                    ...
                    "difference": comparison_metrics["overall_scores"]["difference"]
                })
            ...
            logger.error(f"Error comparing sessions {session1_id} and {session2_id}: {e}")
            return None
    ...
    def _passes_filters(self, session_data: Dict[str, Any], filters: Dict[str, Any]) -> bool:
    ...
    async def _get_session_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if plan_data:
                ...
                metrics["scenarios_completed"] = len([s for s in scenarios if s.get("completed", False)])
    ...
    def _calculate_trend(self, data_points: List[Tuple[datetime, float]]) -> Tuple[str, float]:
