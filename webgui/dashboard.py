...
class DashboardManager:
    ...
    async def get_resource_metrics(self) -> ResourceMetrics:
        ...
        try:
            ...
            active_sessions = len([s for s in await self.get_active_sessions() 
                                  if s.status in [SessionStatus.PLANNING, SessionStatus.TESTING, SessionStatus.ANALYSIS]])
            active_agents = len([a for a in await self.get_agent_status() 
            ...
            return ResourceMetrics(
                total_sessions=0, active_sessions=0, active_agents=0,
                redis_memory_usage=0, redis_connections=0, system_load=0.0, uptime_seconds=0
            )
    ...
    async def export_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sessions": [asdict(session) for session in await self.get_active_sessions()],
