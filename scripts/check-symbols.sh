...
#   1. Every top-level symbol in src/ is `agnostic_*` / `_agnostic_*` / `_`-internal
#      / `AGN*` / `AGNOSTIC_*`, or the entry point `main`. That single rule closes
...
for path in sorted(glob.glob("src/**/*.cyr", recursive=True)):
    # src/test.cyr is a SEPARATE translation unit — cyrius.cyml's [build].test
    # entry, compiled on its own, never alongside src/main.cyr. Its `main`/`r`
    # are not in the same symbol table as the binary's, so scanning it here
    # reports a collision that cannot happen. Excluded deliberately; the suites
    # under tests/*.tcyr are likewise their own units.
    if path == "src/test.cyr":
        continue
    in_enum = None
...
for path in sorted(glob.glob("src/**/*.cyr", recursive=True)):
    ...
    for lineno, line in enumerate(open(path), 1):
        ...
        if name == "main":
            continue
        if name.startswith(("agnostic_", "_agnostic_", "AGNOSTIC_", "AGN_", "_")):
            continue
...
if [ -n "$unprefixed" ]; then
    echo "$unprefixed"
    echo
    echo "error: a top-level symbol in src/ is not agnostic-prefixed."
