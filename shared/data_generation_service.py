...
class UnifiedDataGenerator:
    ...
    def generate_test_data(self, data_type: str, count: int = 10, config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        self.redis_client.setex(cache_key, 3600, json.dumps(generated_data))  # Cache for 1 hour
        ...
        for field_name, field_config in preset.items():
            ...
            if field_type == "string":
                if "pattern" in field_options:
                    item[field_name] = self._generate_pattern_string(field_options["pattern"], index)
                elif "values" in field_options:
                    item[field_name] = random.choice(field_options["values"])
                else:
                    item[field_name] = self.faker.sentence(nb_words=field_options.get("words", 5))
                ...
                if "range" in field_options:
                    min_val, max_val = field_options["range"]
                    item[field_name] = round(random.uniform(min_val, max_val), field_options.get("decimals", 2))
                else:
                    item[field_name] = round(random.uniform(0, 100), 2)
                ...
                if "range" in field_options:
                    ...
                    random_date = start_dt + timedelta(seconds=random.randint(0, int((end_dt - start_dt).total_seconds())))
                    item[field_name] = random_date.isoformat()
                else:
                    item[field_name] = self.faker.date_between(start_date='-30d', end_date='today').isoformat()
            
            elif field_type == "boolean":
                item[field_name] = random.choice([True, False])
            
            elif field_type == "enum":
                item[field_name] = random.choice(field_options.get("values", []))
                ...
                item[field_name] = self._generate_nested_object(field_options.get("schema", {}))
    ...
    def _generate_pattern_string(self, pattern: str, index: int) -> str:
        ...
        patterns = {
            ...
            "uuid": lambda: f"uuid_{index:04d}_{random.randint(1000, 9999)}"
        }
    ...
    def _generate_nested_object(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in schema.items():
            if isinstance(value, dict):
                ...
                obj[key] = [random.choice(value) for _ in range(random.randint(1, 3))]
            else:
                obj[key] = str(value)
        return obj
    ...
    def generate_edge_case_data(self, base_data_type: str, edge_cases: List[str] = None) -> Dict[str, Any]:
        """Generate edge case data for testing"""
        edge_cases = edge_cases or ["boundary", "null", "empty", "max_length", "special_chars"]
    ...
    def _generate_edge_case_item(self, preset: Dict[str, Any], edge_case: str) -> Dict[str, Any]:
        ...
        for field_name, field_config in preset.items():
            ...
            if edge_case == "null":
                ...
                item[field_name] = "" if field_type == "string" else [] if field_type == "array" else {}
                ...
                if field_type in ["number", "float"]:
                    ...
                    item[field_name] = range_vals[0] - 1 if random.choice([True, False]) else range_vals[1] + 1
                else:
                    item[field_name] = self._generate_data_item(preset, 0)[field_name]
            else:
                item[field_name] = self._generate_data_item(preset, 0)[field_name]
    ...
    def _get_api_testing_preset(self) -> Dict[str, Any]:
        return {
            "_name": "api_testing",
            "id": {"type": "number", "required": True, "description": "Unique identifier"},
            "name": {"type": "string", "required": True, "options": {"pattern": "username"}},
            "email": {"type": "email", "required": True},
            "age": {"type": "number", "required": False, "options": {"range": [18, 99]}},
            "active": {"type": "boolean", "required": False},
            "created_at": {"type": "date", "required": False},
            "tags": {"type": "array", "required": False, "options": {"size": 3}},
            "metadata": {"type": "object", "required": False, "options": {"schema": {"source": "api", "version": "1.0"}}}
        }
    ...
    def _get_form_testing_preset(self) -> Dict[str, Any]:
        return {
            ...
            "country": {"type": "enum", "required": False, "options": {"values": ["US", "CA", "UK", "AU"]}},
            "newsletter": {"type": "boolean", "required": False},
            "birth_date": {"type": "date", "required": False, "options": {"range": ["1950-01-01", "2005-12-31"]}},
            "comments": {"type": "string", "required": False, "options": {"max_length": 500}}
        }

    def _get_performance_testing_preset(self) -> Dict[str, Any]:
        return {
            "_name": "performance_testing",
            "endpoint": {"type": "string", "required": True, "options": {"values": ["/api/users", "/api/products", "/api/orders"]}},
            "method": {"type": "enum", "required": True, "options": {"values": ["GET", "POST", "PUT", "DELETE"]}},
            "payload_size": {"type": "number", "required": False, "options": {"range": [0, 10000]}},
            "concurrent_users": {"type": "number", "required": False, "options": {"range": [1, 1000]}},
            "response_time_ms": {"type": "float", "required": False, "options": {"range": [0.1, 10000.0]}},
    ...
    def _get_security_testing_preset(self) -> Dict[str, Any]:
        return {
            ...
            "password": {"type": "string", "required": True, "options": {"max_length": 128}},
            "email": {"type": "email", "required": True},
            "role": {"type": "enum", "required": False, "options": {"values": ["user", "admin", "guest"]}},
            "permissions": {"type": "array", "required": False, "options": {"size": 5}},
            "last_login": {"type": "date", "required": False},
            "failed_attempts": {"type": "number", "required": False, "options": {"range": [0, 10]}}
        }

    def _get_accessibility_testing_preset(self) -> Dict[str, Any]:
        return {
            "_name": "accessibility_testing",
            "element_type": {"type": "enum", "required": True, "options": {"values": ["button", "link", "input", "image", "heading"]}},
            "text": {"type": "string", "required": True, "options": {"max_length": 100}},
            "alt_text": {"type": "string", "required": False, "options": {"max_length": 200}},
            "aria_label": {"type": "string", "required": False},
            "tab_index": {"type": "number", "required": False, "options": {"range": [-1, 100]}},
    ...
    def _get_mobile_testing_preset(self) -> Dict[str, Any]:
        return {
            ...
            "device_type": {"type": "enum", "required": True, "options": {"values": ["mobile", "tablet", "desktop"]}},
            "os": {"type": "enum", "required": True, "options": {"values": ["iOS", "Android", "Windows", "macOS"]}},
            "os_version": {"type": "string", "required": False},
            "screen_width": {"type": "number", "required": False, "options": {"range": [320, 2560]}},
            "screen_height": {"type": "number", "required": False, "options": {"range": [568, 1440]}},
            "orientation": {"type": "enum", "required": False, "options": {"values": ["portrait", "landscape"]}},
            "touch_enabled": {"type": "boolean", "required": False},
            "network_type": {"type": "enum", "required": False, "options": {"values": ["wifi", "4g", "3g", "2g"]}}
        }
    ...
    def _get_database_testing_preset(self) -> Dict[str, Any]:
        return {
            ...
            "operation": {"type": "enum", "required": True, "options": {"values": ["INSERT", "UPDATE", "DELETE", "SELECT"]}},
            "record_id": {"type": "number", "required": False},
            "data": {"type": "object", "required": True, "options": {"schema": {"field1": "value1", "field2": "value2"}}},
            "query_time_ms": {"type": "float", "required": False, "options": {"range": [0.1, 10000.0]}},
            "rows_affected": {"type": "number", "required": False, "options": {"range": [0, 10000]}},
    ...
    def _get_regression_testing_preset(self) -> Dict[str, Any]:
        return {
            ...
            "test_case_id": {"type": "string", "required": True, "options": {"pattern": "id"}},
            "test_name": {"type": "string", "required": True},
            "module": {"type": "string", "required": True},
            "priority": {"type": "enum", "required": False, "options": {"values": ["high", "medium", "low"]}},
            "expected_result": {"type": "string", "required": True},
            "actual_result": {"type": "string", "required": False},
            "status": {"type": "enum", "required": False, "options": {"values": ["pass", "fail", "skip"]}},
            "execution_time_ms": {"type": "float", "required": False, "options": {"range": [0.1, 300000.0]}},
            "environment": {"type": "string", "required": False, "options": {"values": ["dev", "staging", "prod"]}}
        }
...
class DataOptimizationService:
    ...
    def _optimize_for_security_compliance(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if config.get("test_gdpr", False):
            base_config["gdpr_data"] = self.generator.generate_test_data("gdpr_testing", 20)
    ...
    def _optimize_for_junior(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "regression_data": self.generator.generate_test_data("regression_testing", 50),
    ...
    def _generate_wcag_test_cases(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"criterion": "2.1.1", "test": "Keyboard accessibility", "elements": ["button", "link", "input"]},
            {"criterion": "3.3.2", "test": "Form labels", "elements": ["input", "select", "textarea"]},
            {"criterion": "4.1.2", "test": "ARIA roles", "elements": ["nav", "main", "section"]}
        ]
    ...
    def _generate_complex_scenarios(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"scenario": "circular_references", "complexity": "medium", "data_type": "object"},
            {"scenario": "unicode_handling", "complexity": "medium", "data_type": "string"}
        ]
...
class DataGenerationService:
    ...
    def generate_for_agent(self, agent_type: str, task_config: Dict[str, Any]) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    # Example usage
    result = service.generate_for_agent("performance", {
